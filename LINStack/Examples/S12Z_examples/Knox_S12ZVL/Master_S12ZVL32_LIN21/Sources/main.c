...
void tim1_init(void) {
	/*Timer prescaler = 1 */
	TIM1TSCR2 = 0x00;
	TIM1TIOS_IOS0 = 1;		/*Enable Channel 0 as Output Compare */
	TIM1TIOS_IOS1 = 1;		/*Enable Channel 1 as Output Compare */
	TIM1TCTL2 = 0x0; 		/*Channel 0 & 1 Output Compare Action = Toggle output line */
	TIM1TIE_C0I = 1;		/*Enable Channel 0 interrupt */
	/* Timer interrupts every 5ms */
	TIM1TC0 = (MCU_BUS_FREQ/1000000)*5000;
...
void main(void) {
    ...
    /* Enable LIN PHY*/
    LP0CR_LPE = 1;
    /* Slew Rate for 9600 baudrate */
    LP0SLRM = 0x01;

    ret = l_sys_init();
    ret = l_ifc_init(LI0);
    // added code to enable linphy in master mode in lin_lld_sci.c file (function lin_lld_sci_init)
...
interrupt VectorNumber_Vtim1ch0 void TIM1chan0_ISR(void)
{
	/* Timer interrupt every 5ms */
    TIM1TC0 += ((MCU_BUS_FREQ/1000000)*5000)& 0xFFFF;
